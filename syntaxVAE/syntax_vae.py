...
from submodules.base_decoder import RNNDecoder
from submodules.base_encoder import RNNEncoder
from submodules.bridge import Bridge
from syntaxVAE.base_vae import BaseVAE
from utils.nn_utils import bag_of_word_loss
from utils.nn_utils import id2word
from utils.nn_utils import noise_input
from utils.nn_utils import to_input_variable
from utils.nn_utils import to_var
from utils.nn_utils import wd_anneal_function
...
class SyntaxVAE(BaseVAE):
    ...
    def batch_beam_decode(self):
        pass

    def __init__(self, args, vocab, src_embed=None, tgt_embed=None):
        super(SyntaxVAE, self).__init__(args, vocab, name="Split Encode VAE")
    ...
    def get_gpu(self):
        model_list = [self.word_encoder, self.bridger, self.decoder, self.syn_mean, self.syn_logv, self.syn_to_h, self.sem_mean, self.sem_logv,
                      self.sem_to_h, self.sup_syn, self.sup_sem, self.syn_adv, self.syn_infer, self.sem_adv, self.sem_infer]
    ...
    def encode(self, input_var, length):
        if self.training and self.args.src_wd > 0.:
            input_var = noise_input(input_var, self.step_unk_rate, self.vocab.src)
    ...
    def forward(self, examples, is_dis=False):
        ...
        tgt_var = to_input_variable(words, self.vocab.src, training=False, cuda=self.args.cuda, append_boundary_sym=True, batch_first=True)
        syn_seqs = [e.tgt for e in examples]
        syn_var = to_input_variable(syn_seqs, self.vocab.tgt, training=False, cuda=self.args.cuda, append_boundary_sym=True, batch_first=True)
        ...
        if self.training and self.args.tgt_wd:
            input_var = noise_input(tgt_var, self.step_unk_rate, self.vocab.src)
    ...
    def get_loss(self, examples, step, is_dis=False):
        self.step_unk_rate = wd_anneal_function(unk_max=self.unk_rate, anneal_function=self.args.unk_schedule, step=step, x0=self.args.x0,
                                                k=self.args.k)
        ...
        sem_kl, kl_weight = self._kl_loss(
            mean=explore['sem_mean'],
            logv=explore['sem_logv'],
            step=step,
        )
        syn_kl, _ = self._kl_loss(
        ...
        kl_loss = (self.args.sem_weight * sem_kl + self.args.syn_weight * syn_kl) / (self.args.sem_weight + self.args.syn_weight)
        kl_loss /= batch_size
    ...
    def get_adv_loss(self, syntax_hidden, semantic_hidden, syn_tgt, sem_tgt):
        if self.training:
            ...
            if self.args.infer_weight > 0.:
                adv_syn = self.args.adv_syn * loss_dict['adv_syn_sup'] + self.args.infer_weight * self.args.inf_sem * loss_dict['adv_sem_inf']
                adv_sem = self.args.adv_sem * loss_dict['adv_sem_sup'] + self.args.infer_weight * self.args.inf_syn * loss_dict['adv_syn_inf']
            else:
    ...
    def get_dis_loss(self, syntax_hidden, semantic_hidden, syn_tgt, sem_tgt):
        ...
        if self.args.infer_weight > 0.:
            return loss_dict['adv_syn_sup'] + loss_dict['adv_sem_inf'], loss_dict['adv_sem_sup'] + loss_dict['adv_syn_inf']
        else:
