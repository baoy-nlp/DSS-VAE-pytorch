...
from models import init_create_model
from models import load_static_model
...
def create_model(main_args, model_args, model=None):
    if model is None:
...
#     args = params['args']
#     vocab = params['vocab']
#     model = build_initial_model(model_args.model_select, args=args, vocab=vocab)
#     model.load_state_dict(params["state_dict"])
#     print(model)
#     if main_args.cuda:
#         # device = torch.device("cuda:0" if torch.cuda.is_available else "cpu")
